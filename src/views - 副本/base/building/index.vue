...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="楼栋ID">
        <template slot-scope="scope">{{ scope.row.id }}</template>
      </el-table-column>
      <el-table-column align="center" label="楼栋号">
        <template slot-scope="scope">{{ scope.row.ldh }}</template>
      </el-table-column>
      <el-table-column align="center" label="建设工程规划许可证编号">
        <template slot-scope="scope">{{ scope.row.xkz }}</template>
      </el-table-column>
      <el-table-column align="center" label="工程名称">
        <template slot-scope="scope">{{ scope.row.gcmc }}</template>
      </el-table-column>
      <el-table-column align="center" label="建筑用途">
        <template slot-scope="scope">{{ scope.row.jzyt }}</template>
      </el-table-column>
      <el-table-column align="center" label="建筑结构类型">
        <template slot-scope="scope">{{ scope.row.type }}</template>
      </el-table-column>
      <el-table-column align="center" label="建筑层数(地上)">
        <template slot-scope="scope">{{ scope.row.num1 }}</template>
      </el-table-column>
      <el-table-column align="center" label="建筑层数(地下)">
        <template slot-scope="scope">{{ scope.row.num2 }}</template>
      </el-table-column>
      <el-table-column align="center" label="计划开工日期">
        <template slot-scope="scope">{{ scope.row.t1 }}</template>
      </el-table-column>
      <el-table-column align="center" label="计划施工日期">
      ...
      <el-table-column align="center" label="底层面积(㎡)">
        <template slot-scope="scope">{{ scope.row.num4 }}</template>
      </el-table-column>
...
<script>
import {getList} from '@/api/base/building'
export default {
  data() {
    return {
      list: null,
