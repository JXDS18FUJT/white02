  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="施工方案">
        <template slot-scope="scope">{{ scope.row.fantype }}</template>
      </el-table-column>
      <el-table-column align="center" label="评审状态">
        <template slot-scope="scope">{{ scope.row.fanzd }}</template>
      </el-table-column>
      <el-table-column align="center" label="评审说明">
        <template slot-scope="scope">{{ scope.row.fanzd }}</template>
      </el-table-column>
      <el-table-column align="center" label="受理状态">
        <template slot-scope="scope">{{ scope.row.state3}}</template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="300" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button type="success" size="mini" @click="handleUpdate(row)">修改</el-button>
          <el-button type="success" size="mini" @click="handleUpdate(row)">评审</el-button>
        </template>
      </el-table-column>
    </el-table>
